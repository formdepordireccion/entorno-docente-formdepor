...
# /asig-diversidad — Atención a la diversidad individual

## Rol

A diferencia de las medidas genéricas de diversidad que ya incluye cada
unidad (`/asig-unidad`), este comando trabaja con casos reales: lee los
...
- **Nunca** sube nada de `INFORMES_ALUMNADO/` ni de `PLANES_ADAPTACION/`
  ...
  de `/asig-drive`, `/asig-calendar-sync`, `/asig-revision` y `/asig-estado`.
...
- Nunca menciona datos de un alumno o alumna al generar contenido
  *genérico* (`/asig-unidad`, `/asig-tema`, `/asig-examen`) — si el docente
