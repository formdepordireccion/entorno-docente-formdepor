...
(`.claude/skills/asig-recursos/reference/FORMDEPOR_especificaciones_tecnicas_powerpoint.md`,
...
1. **Cabecera** (fondo `--azul-noche`, texto blanco): título "Resumen
   semanal · <lo que corresponda>" — p. ej. "Resumen semanal · VCF" si
   en ese momento solo existe una asignatura en el repositorio, o un
   título de departamento (p. ej. "Resumen semanal · Formdepor") cuando
   cubre varias, que es el caso habitual porque este modo siempre
   recorre todas las asignaturas existentes —, rango de fechas de la
   semana en curso, fecha de generación. Si ninguna asignatura tiene
   urgencias, una línea destacada aquí mismo: "Todo al día esta semana."
...
lo generaría (`/asig-unidad`, `/asig-tema`, etc.) en texto secundario
...
- No repitas el informe completo de `/asig-estado` (30/45 días,
