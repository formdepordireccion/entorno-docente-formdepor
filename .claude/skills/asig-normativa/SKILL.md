...
aprendizaje, criterios, contenidos y horas del módulo de la asignatura resuelta (ver CLAUDE.md → "Cómo se resuelve la asignatura") dentro del
título correspondiente.

## Entradas

- `ficha.yaml` (para saber qué campos de normativa ya están rellenos)
- Título: el de la asignatura resuelta (nombre y código en su `ficha.yaml → asignatura.titulo_ciclo`).
- Módulo: el de la asignatura resuelta (nombre y código en su `ficha.yaml → asignatura`).
...
- Programación docente real, si existe: `02_PROGRAMACION/programacion_26_27.md`
  ...
  (por `/asig-auditoria`) — ver "Orden de prioridad de fuentes" más abajo.
...
1. Con WebSearch, busca en boe.es el Real Decreto que regula el título
   correspondiente y localiza dentro de él el módulo profesional de la
   asignatura resuelta (comprueba el nombre oficial exacto del módulo, puede diferir ligeramente).
...
3. Con WebSearch, busca en el Diario Oficial de Extremadura (DOE) el
   decreto autonómico que desarrolla el currículo correspondiente en Extremadura,
   y localiza las horas/contenidos específicos de la asignatura si los hay.
...
son los correctos antes de que `/asig-programacion`, `/asig-unidad`, `/asig-tema`
o `/asig-examen` usen estos datos.
