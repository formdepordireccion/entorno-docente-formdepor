...
2. Convierte ese esquema (no el documento original) a `.pptx` con el
   ...
   .venv-recursos/bin/python .claude/skills/asig-recursos/scripts/generar_pptx.py \
     ...
     --subtitulo "<CODIGO> · <CICLO> · Formdepor"
   ```
   `--subtitulo` es opcional (por defecto "Formdepor" a secas); rellena
   `<CODIGO>` y `<CICLO>` con los de la asignatura resuelta (ver
   CLAUDE.md → "Cómo se resuelve la asignatura") para que la portada
   identifique la asignatura correcta.
...
2. Llama a la generación de Gamma con `format: presentation`,
   ...
   estructurado en secciones (lo habitual viniendo de `/asig-unidad` o
   `/asig-tema`), `language: es`, y `textOptions.audience`: "alumnado de
